...
//请补充代码
    for(i=0;i<4;i++){
        if (i==0){
            int temp1,temp2;
            leftLoop4int(numArray[3],1);
            temp1=getNumFromSBox(numArray[3]);
            temp2=temp1^Rcon[round];
            newarray[i]=numArray[i]^temp2;
        }else{
            newarray[i]=numArray[i]^newArray[i-1];
        }
    }
    for(int j=0;j<4;j++){
        result+=(newarray[j]<<(j*8))
    }
    return result;
}
