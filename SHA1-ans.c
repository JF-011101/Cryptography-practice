...
void shaTran(unsigned char * buf,unsigned int * state)
{
    ...
    {
        //TODO  需要添加代码
        temp=(A<<5|A>>27)+f1(B,C,D)+E+W[i]+K[0];
        E=D;
        D=C;
        C=B<<30|B>>2;
        B=A;
    ...
    {
       //TODO  需要添加代码
       temp=(A<<5|A>>27)+f2(B,C,D)+E+W[i]+K[1];
        E=D;
        D=C;
        C=B<<30|B>>2;
        B=A;
    ...
    {
       //TODO  需要添加代码
       temp=(A<<5|A>>27)+f3(B,C,D)+E+W[i]+K[2];
        E=D;
        D=C;
        C=B<<30|B>>2;
        B=A;
    ...
    {
       //TODO  需要添加代码
       temp=(A<<5|A>>27)+f4(B,C,D)+E+W[i]+K[3];
        E=D;
        D=C;
        C=B<<30|B>>2;
        B=A;
...
void sha(unsigned char *buf, int len, unsigned int * state, unsigned int f1,unsigned int f2)
{
    ...
    else
    {
        ...
        shaTran(buf,state);//添加到64个字节再使用SHA-1算法
        ...
        for(i=56; i<64; i++)
        {
            buf[i] = flen[i-56];//原明文的长度
        }

        /*call the function "shaTran" to do the loop*/
        shaTran(buf,state);//最后512bit使用SHA-1算法
    }
}
/*********************************the end of the function*/
